...
assertEqual(setDefaultResult.is_default, 1, '设为默认地址成功');
...
assert(cartData.find(i => i.product_id === productForCart.id).quantity === 3, '购物车数量正确');

const cartAfterUpdate = cartModel.updateQuantity(cartData[0].id, 5, 1);
const updated = cartAfterUpdate.find(i => i.product_id === productForCart.id);
...
const removed = cartAfterRemove.find(i => i.product_id === productForCart.id);
...
assert(order.order_no.startsWith('ORD'), '订单编号格式正确');
assert(order.items.length === 1, '订单包含 1 个明细');
assertEqual(order.items[0].total_price, 200, '订单明细金额正确');
